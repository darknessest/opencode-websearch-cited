...
If you specify multiple `websearch_cited.model` fields in your `opencode.json`, the plugin routes each search to the provider of the model that called the tool, so an anthropic model searches with the anthropic block and an openai model searches with the openai block.
